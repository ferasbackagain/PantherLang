...
# Purpose: Run historical H4 regression suites against the production debug_adapter,
#          generate compatibility matrix, classify failures, and preserve rollback capability.
# Policy:  No monkey patches. No quick fixes. No production mutation.

BATCH_NAME="P3_batch7_full_h4_compatibility_regression"
SCRIPT_NAME="bootstrap_P3_batch7_full_h4_compatibility_regression.sh"
TS="$(date +%Y%m%d_%H%M%S)"
ROOT="$(pwd)"
ARTIFACT_ROOT="$ROOT/.panther/p3_batch7_h4_compatibility/$TS"
REPORT_DIR="$ROOT/reports/p3_batch7_h4_compatibility_$TS"
LOG_DIR="$ARTIFACT_ROOT/logs"
SNAPSHOT_DIR="$ARTIFACT_ROOT/snapshots"
TMP_DIR="$ARTIFACT_ROOT/tmp"
MANIFEST="$ARTIFACT_ROOT/h4_regression_manifest.json"
RUNNER="$ARTIFACT_ROOT/p3_batch7_runner.py"

export PYTHONUNBUFFERED=1
export PYTHONDONTWRITEBYTECODE=1
export PANTHER_P3_BATCH7_READONLY=1

say() { printf '\n\033[1;36m[%s]\033[0m %s\n' "$BATCH_NAME" "$*"; }
warn() { printf '\n\033[1;33m[%s:WARN]\033[0m %s\n' "$BATCH_NAME" "$*"; }
fail() { printf '\n\033[1;31m[%s:FAIL]\033[0m %s\n' "$BATCH_NAME" "$*"; exit 1; }
...
hash_path() {
  if [ -e "$1" ]; then
    if command -v sha256sum >/dev/null 2>&1; then
      if [ -d "$1" ]; then
        find "$1" -type f -not -path '*/__pycache__/*' -print0 | sort -z | xargs -0 sha256sum | sha256sum | awk '{print $1}'
      else
...
say "Starting PantherLang P-3 Batch 7 H4 compatibility regression"
...
mkdir -p "$ARTIFACT_ROOT" "$REPORT_DIR" "$LOG_DIR" "$SNAPSHOT_DIR" "$TMP_DIR"

# Safety gate: must be run from a PantherLang project root.
[ -d "$ROOT/debug_adapter" ] || fail "debug_adapter/ not found. Run this script from PantherLang project root."
[ -d "$ROOT/tests" ] || fail "tests/ not found. Run this script from PantherLang project root."

# Verify production adapter state.
say "Verifying production debug_adapter and rollback capability"
PROD_HASH_BEFORE="$(hash_path "$ROOT/debug_adapter")"
REBUILT_HASH="$(hash_path "$ROOT/debug_adapter_rebuilt" 2>/dev/null || true)"
LEGACY_COUNT="$(find "$ROOT" -maxdepth 1 -type d -name 'debug_adapter_legacy_P3_*' | wc -l | tr -d ' ')"
ROLLBACK_CANDIDATES="$(find "$ROOT/.panther" "$ROOT" -maxdepth 4 \( -iname '*rollback*' -o -iname 'rollback_*.sh' \) 2>/dev/null | wc -l | tr -d ' ')"

[ "$PROD_HASH_BEFORE" != "missing" ] || fail "Production debug_adapter hash could not be computed."
[ "$LEGACY_COUNT" -gt 0 ] || fail "No debug_adapter_legacy_P3_* directory found. Rollback lineage is missing."
[ "$ROLLBACK_CANDIDATES" -gt 0 ] || fail "No rollback package/script found. Rollback capability is missing."

if [ -d "$ROOT/debug_adapter_rebuilt" ] && [ "$PROD_HASH_BEFORE" = "$REBUILT_HASH" ]; then
  PROMOTION_STATUS="production_matches_rebuilt"
else
  PROMOTION_STATUS="production_present_rebuilt_hash_diff_or_absent"
  warn "debug_adapter_rebuilt hash differs or is absent. This is recorded, not fixed."
fi

# Snapshot read-only evidence. No production mutation.
say "Creating read-only production snapshot"
tar -czf "$SNAPSHOT_DIR/debug_adapter_production_$TS.tar.gz" -C "$ROOT" debug_adapter
find "$ROOT" -maxdepth 1 -type d -name 'debug_adapter_legacy_P3_*' -print0 | \
  tar --null -czf "$SNAPSHOT_DIR/debug_adapter_legacy_refs_$TS.tar.gz" -C "$ROOT" --files-from - 2>/dev/null || true

cat > "$ARTIFACT_ROOT/environment.json" <<JSON
{
  "batch": "$BATCH_NAME",
  "timestamp": "$TS",
  "project_root": "$ROOT",
  "production_debug_adapter_hash_before": "$PROD_HASH_BEFORE",
  "debug_adapter_rebuilt_hash": "$REBUILT_HASH",
  "promotion_status": "$PROMOTION_STATUS",
  "legacy_adapter_count": $LEGACY_COUNT,
  "rollback_candidate_count": $ROLLBACK_CANDIDATES,
  "policy": {
...
say "Writing Batch 7 regression runner"
cat > "$RUNNER" <<'PY'
#!/usr/bin/env python3
...
say "Running full H4 compatibility regression against production debug_adapter"
export PANTHER_ROOT="$ROOT"
export PANTHER_BATCH7_ARTIFACT_ROOT="$ARTIFACT_ROOT"
export PANTHER_BATCH7_REPORT_DIR="$REPORT_DIR"
export PANTHER_BATCH7_LOG_DIR="$LOG_DIR"
export PANTHER_BATCH7_MANIFEST="$MANIFEST"

set +e
python3 "$RUNNER" | tee "$ARTIFACT_ROOT/runner_console.log"
RUNNER_EXIT=${PIPESTATUS[0]}
set -e

PROD_HASH_AFTER="$(hash_path "$ROOT/debug_adapter")"
if [ "$PROD_HASH_BEFORE" != "$PROD_HASH_AFTER" ]; then
  fail "Production debug_adapter changed during regression. This violates Batch 7 read-only policy. Roll back before continuing."
fi

# Copy environment evidence into report dir after after-hash was written.
cp -f "$ARTIFACT_ROOT/environment.json" "$REPORT_DIR/environment.json"
cp -f "$ARTIFACT_ROOT/runner_console.log" "$REPORT_DIR/runner_console.log"
[ -f "$MANIFEST" ] && cp -f "$MANIFEST" "$REPORT_DIR/h4_regression_manifest.json" || true

say "Batch 7 reports generated"
printf '\nReports directory:\n  %s\n' "$REPORT_DIR"
printf '\nKey reports:\n  %s\n  %s\n  %s\n  %s\n' \
  "$REPORT_DIR/compatibility_matrix.md" \
  "$REPORT_DIR/compatibility_matrix.json" \
  "$REPORT_DIR/failure_classification.json" \
  "$REPORT_DIR/engineering_report.md"

if [ "$RUNNER_EXIT" -eq 0 ]; then
  say "P-3 Batch 7 COMPLETE: H4 compatibility regression executed. Review regression_summary.json for Batch 8 readiness."
else
  warn "P-3 Batch 7 executed, but blocking findings or discovery issues were recorded. Review engineering_report.md and failure_classification.json."
fi

# The script exits 0 if the regression infrastructure completed and production remained unchanged.
# Test failures are recorded in reports; they do not mutate production or trigger quick fixes.
exit 0
