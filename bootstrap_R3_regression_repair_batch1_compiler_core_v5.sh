...
echo "=================================================="
echo "PantherLang R3 Regression Repair - Batch 1 v5"
echo "compiler.core Compatibility Layer"
echo "Fix: v0.5 model node contract"
echo "=================================================="

mkdir -p .panther/backups .panther/manifests docs/compiler_runtime/reports
BACKUP=".panther/backups/R3_REGRESSION_REPAIR_BATCH1_COMPILER_CORE_V5_${STAMP}"
mkdir -p "$BACKUP"
...
p = parse_tokens([])
assert p.nodes[0].kind == "model"
assert p.nodes[0].name == "Product"
print("parse_tokens v5 smoke passed")
PY
...
cat > .panther/manifests/r3_regression_repair_batch1_compiler_core_v5_manifest.json <<JSON
{
  "stage": "R3 Regression Repair Batch 1 v5",
  "status": "PASSED",
  "timestamp": "${STAMP}",
  "backup": "${BACKUP}",
...
cat > docs/compiler_runtime/reports/r3_regression_repair_batch1_compiler_core_v5_report.md <<MD
# R3 Regression Repair Batch 1 v5

Fixed compiler.core v0.5 model contract.
...
echo "R3 Regression Repair Batch 1 v5 completed successfully."
