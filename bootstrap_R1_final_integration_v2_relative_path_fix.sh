...
echo " Final Integration v2 - Relative Path Fix"
echo "============================================================"
...
fail(){ echo "[R1-FINAL-v2][ERROR] $1" >&2; exit 1; }
...
echo "[2/12] Resolving VSIX as absolute path..."
VSIX="$(python3 - <<'PY'
import json
...
if candidate.exists():
    ...
    matches = sorted((root / "releases/vscode_marketplace").glob("pantherlang-1.0.0*.vsix"), key=lambda p: p.stat().st_mtime, reverse=True)
    print(matches[0].resolve() if matches else "")
PY
)"
