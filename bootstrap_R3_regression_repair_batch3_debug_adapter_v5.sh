...
echo "=================================================="
echo "PantherLang R3 Regression Repair - Batch 3 v5"
echo "Fix: protocol exports expected by debug_adapter.__init__"
echo "=================================================="

mkdir -p .panther/backups .panther/manifests docs/compiler_runtime/reports
BACKUP=".panther/backups/R3_REGRESSION_REPAIR_BATCH3_DEBUG_ADAPTER_V5_${STAMP}"
mkdir -p "$BACKUP"
...
echo "[1/4] Smoke-check protocol exports..."
python3 - <<'PY'
from debug_adapter.protocol import (
    DAPProtocol,
    DAPProtocolError,
    DAPEncodedMessage,
    DAPDecodedMessage,
...
assert str(DAPEncodedMessage("x")) == "x"

print("protocol v5 smoke passed")
PY
...
print("debug_adapter v5 public imports smoke passed")
PY

echo "[3/4] Running R3 parser regression..."
python3 -m pytest -q tests/R3_compiler_runtime

cat > .panther/manifests/r3_regression_repair_batch3_debug_adapter_v5_manifest.json <<JSON
{
  "stage": "R3 Regression Repair Batch 3 v5 - Debug Adapter Compatibility",
  "status": "PASSED",
  "timestamp": "${STAMP}",
  "backup": "${BACKUP}",
...
cat > docs/compiler_runtime/reports/r3_regression_repair_batch3_debug_adapter_v5_report.md <<MD
# R3 Regression Repair Batch 3 v5

Repaired \`debug_adapter.protocol\` exports required by \`debug_adapter.__init__\`.
...
echo "[4/4] Completed."
echo "R3 Regression Repair Batch 3 v5 completed successfully."
