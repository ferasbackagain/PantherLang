...
def test_unified_version_core_contract():
    assert PANTHERLANG_VERSION == "1.1.9"
    assert PANTHERLANG_RELEASE_NAME == "PantherLang v1.1.9"
    assert get_version() == "1.1.9"
    info = get_release_info()
    assert info["product"] == "PantherLang"
    assert info["version"] == "1.1.9"
    assert info["debug_adapter_version"] == "1.1.9"


def test_cli_and_runtime_version_bridge():
    assert cli_get_version() == "1.1.9"
    assert runtime_get_version() == "1.1.9"