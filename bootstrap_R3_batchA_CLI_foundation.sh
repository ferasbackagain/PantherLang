...
backup_file "panther"
backup_file ".panther/tools/panther_cli.py"

cat > "$ROOT/.panther/tools/panther_cli.py" <<'PY'
#!/usr/bin/env python3
...
PY
chmod +x "$ROOT/.panther/tools/panther_cli.py"

cat > "$ROOT/panther" <<'SH2'
#!/usr/bin/env bash
set -euo pipefail
...
# Ensure basic project templates are present for VS Code/project wizard and local CLI tests.
python3 "$ROOT/.panther/tools/panther_cli.py" doctor >/tmp/panther_batchA_doctor.txt
...
cat > "$REPORT_DIR/MANIFEST.json" <<EOF
{
  ...
  "purpose": [
    "restore repository-local ./panther executable",
    "support run/check/compile/build/test/new commands",
    "support local project build manifest for phase9_1 tests",
    "ensure project templates exist"
  ],
  "backup": "$BACKUP_DIR",
  "report": "$REPORT_DIR"
}
...
Installed a repository-local \`./panther\` executable and a deterministic Python CLI facade.

## Scope

- CLI executable restoration
- \`run\`, \`check\`, \`compile\`, \`build\`, \`test\`, \`new\`, \`doctor\`
- Local project build support
- Basic template availability guarantee

## Not in scope

- Debug Adapter Protocol contract fixes
- VS Code debugger contribution fixes
- Variables/watch/scopes compatibility facades

Those belong to Batch B/C.
EOF

echo "R3 Batch A CLI Foundation applied."
echo "Backup: $BACKUP_DIR"
echo "Report: $REPORT_DIR"
echo "Now run the Batch A targeted regression command from README_RUN_ORDER.md"
